...
class LiveBodyDetection:
    ...
    def __init__(self, config, debug=False):
        ...
        self._motion_timeout_seconds = (
            utils.dehumanize(lbd_conf["timeout"])
            if "timeout" in lbd_conf
            else self.DEFAULT_MOTION_TIMEOUT
        )
        ...
        if "on_motion_trigger" in lbd_conf:
            if "hsbk" in lbd_conf["on_motion_trigger"]:
                self._on_motion_trigger_hsbk = utils.parse_hsbk_map(
                    lbd_conf["on_motion_trigger"]["hsbk"]
                )

            if "transition" in lbd_conf["on_motion_trigger"]:
                self._on_motion_trigger_transition = utils.dehumanize(
                    lbd_conf["on_motion_trigger"]["transition"]
                )
        ...
        if "on_timeout" in lbd_conf:
            if "hsbk" in lbd_conf["on_motion_timeout"]:
                self._on_motion_timeout_hsbk = utils.parse_hsbk_map(
                    lbd_conf["on_motion_timeout"]["hsbk"]
                )

            if "transition" in lbd_conf["on_motion_timeout"]:
                self._on_motion_timeout_transition = utils.dehumanize(
                    lbd_conf["on_motion_timeout"]["transition"]
                )
    ...
    def process(self):
        motion_detection = self.motion_sensor.motion
