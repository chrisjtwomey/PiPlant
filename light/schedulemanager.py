...
class ScheduleManager:
    ...
    def __init__(self, config, debug=False):
        ...
        city_loc_name = (
            config["geo_city"] if "geo_city" in config else self.GEO_DEFAULT_CITY
        )
        self.geotz = (
            config["geo_tz"].upper() if "geo_tz" in config else self.GEO_DEFAULT_TZ
        )
        self.geocity = lookup(city_loc_name, database())

        self._cache_refresh_timeout = (
            utils.dehumanize(config["device_query_interval"])
            if "device_query_interval" in config
            else self.DEFAULT_CACHE_REFRESH_TIMEOUT
        )
        ...
        if discovery_enabled:
            ...
            self._devicegroups = self._get_device_groups_from_config(
    ...
    def process(self):
        ...
        if "device_groups" not in current_schedule:
            raise ValueError(
                "No device groups specified for current schedule: {}".format(
                    current_schedule
                )
            )
    ...
    def _get_device_groups_from_config(self, config_devicegroups):
        ...
        for group_entry in config_devicegroups:
            for group_entry_name, device_entries in group_entry.items():
                ...
                group_name, group = self.init_devicegroup_from_config(
                    group_entry_name, devices
                )
    ...
    def _get_current_schedule(self):
        ...
        for _, schedule in enumerate(self._schedules):
            schedule_dt = utils.hour_to_datetime(
                schedule["time"], tz=self.geocity.tzinfo
            )
            transition_seconds = (
    ...
    def _get_devicegroup_state(self, group_names=[], use_local_state=True):
        if self.debug:
            self.log.debug(
                "get_devicegroup_state:\n\tgroup_names: {}\n\tuse_local_state: {}".format(
                    group_names, use_local_state
                )
            )
            return dict()
    ...
    def _set_devicegroup_hsbk(self, hsbk, group_names=[], transition_seconds=0):
        if self.debug:
            self.log.debug(
                "_set_devicegroup_hsbk:\n\tgroup_names: {}\n\tHSBK: {}\n\ttransition_seconds: {}".format(
                    group_names, hsbk, transition_seconds
                )
            )
            return
